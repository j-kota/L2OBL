...
parser.add_argument('--predpos',    type=int,     default=0)
...
softplus = torch.nn.Softplus(beta=0.5)
B_ub_flat = torch.flatten(B)
B_mask = (B != 0.0).float()
B_mask_flat = torch.flatten(B_mask)

def B_restore(B):
    return relu(B)*B_mask
...
if   (args.predpos==1):
    ...
    print("Using no final layer")
    def B_sigmoid(pred):
        return pred


input_size  = nx + (nsteps+1)*(ny)
B_predictor = Nets.ReLUnet(input_size, nB, hidden_sizes = [(input_size), 4*(input_size), 8*(input_size), 16*(input_size), 8*(input_size), 4*(input_size), (input_size), nB], batch_norm = True, initialize = True)
...
def predict_correct_solve(ymin, ymax, d, x0,   n_corr_steps = 2):
    ...
    pred_input = torch.cat( (ymin_flat, x0_flat), dim=1 )
...
for epoch in range(epochs+1):
    ...
    batch_viol_norm_list      = []
    batch_viol_mean_list      = []
    ...
    for (ymin, ymax, d, x0) in B_test_loader:
        ...
        end_time   = time.time()
        print("Time = {}".format( (end_time-start_time)/len(ymin)   ))
        ...
        viol_mean = torch.cat((viols_ymin, viols_ymax), dim = 1).mean(1)
    ...
    test_viol_norm_avg      = torch.cat( tuple(batch_viol_norm_list), dim=0 ).mean().item()
    ...
    test_viol_mean_avg      = torch.cat( tuple(batch_viol_mean_list), dim=0 ).mean().item()
    ...
    test_viol_norm_std      = torch.cat( tuple(batch_viol_norm_list), dim=0 ).std().item()
    ...
    test_viol_mean_std      = torch.cat( tuple(batch_viol_mean_list), dim=0 ).std().item()
...
test_viol_norm_up   = (np.array(test_viol_norm_list) + np.array(test_viol_norm_std_list)).tolist()
test_viol_norm_dwn  = (np.array(test_viol_norm_list) - np.array(test_viol_norm_std_list)).tolist()
axs[0].semilogy(range(len(test_viol_norm_list)), test_viol_norm_list, 'b-', label=r"$\|viol\|_2$")
axs[0].fill_between(range(len(test_viol_norm_list)), test_viol_norm_dwn, test_viol_norm_up, color = 'lightskyblue')
axs[0].legend(fontsize=14)

test_viol_mean_up   = (np.array(test_viol_mean_list) + np.array(test_viol_mean_std_list)).tolist()
test_viol_mean_dwn  = (np.array(test_viol_mean_list) - np.array(test_viol_mean_std_list)).tolist()
axs[1].semilogy(range(len(test_viol_mean_list)), test_viol_mean_list, 'k-', label=r"$\frac{1}{N} \sum viol$")
axs[1].fill_between(range(len(test_viol_mean_list)), test_viol_mean_dwn, test_viol_mean_up, color = 'lightslategrey')
axs[1].set_ylabel('Coupling Constraint Violations: Test Set')
...
axs[2].plot(range(len(test_Bsum_list)), test_Bsum_list, 'b-', label=r"$\Sigma \; B$")
...
axs[3].semilogy(range(len(test_loss_list)), test_loss_list, label = "Loss")
axs[3].set_ylabel('Loss Function: Test Set')
axs[3].set_xlabel('Training Epoch')
...
output_dict["test_viol_norm_list"] = test_viol_norm_list
...
output_dict["test_viol_norm_std_list"] = test_viol_norm_std_list
